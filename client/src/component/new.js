...
export default class memberPannel extends Component {
  ...
  generatePDF(details) {
    const doc = new jspdf();
    const tableColumn = ["Item Name", "Date", "Quantity", "Price","Total"];
    const tableRows = [];

    details.map((detail) => {
      const memberDetail = [detail.itemname, detail.date, detail.quantity, detail.price, detail.total];
  ...
  generatePDFFamily(type) {
    ...
    countTypes.map((detail) => {
      const memberDetail = [detail.itemname, detail.date, detail.quantity, detail.price, detail.price];
      tableRows.push(memberDetail);
    });
    doc.text("Profit Report", 14, 15).setFontSize(12);
    doc.addImage(Logo, "JPEG", 115, 5, 80, 30);
